...
*/
		...
		// Point2D.Double((person.x+10)*Math.sin(person.angle),
		// (person.y+10)*Math.sin(person.angle)).distance(person.nearestPlanet.centre)));
		...
		if (iskeyDown(KeyEvent.VK_UP)) {
			bullets.add(new Bullet(person.x, person.y, person.velX + 4 * Math.sin(person.shootingAngle),
					person.velY + 4 * Math.cos(person.shootingAngle)));
		}
	...
	public void addEnemy() {
		enemies.add(new Enemy(person.x + Helper.randInt(-200, 200), person.y + Helper.randInt(-200, 200),
				Helper.randInt(-5, 5), Helper.randInt(-5, 5)));
	}

	public void addPlanetSafely() {
		Planet newPlanet = new Planet((double) Helper.randInt(-1000, 1000), (double) Helper.randInt(-1000, 1000),
				(double) Helper.randInt(50, 200), (double) (Helper.randInt(10, 30)) / 10,
				Helper.randomrainbowcolor(.6f));
