...
public class Enemy extends Pixel {
	...
	BufferedImage enemyImg = new Image("img/spoopy.png").img;
	...
	public void drawColumns(Graphics2D g2){
		...
		for(Rectangle current: columns){
			//g2.fill(current);
			g2.drawImage(comp.getSlice(enemyImg, columns.indexOf(current), columns.size()),current.x,current.y,current.width,current.height,null);
		}
	}
	public void move() {
		amountHit = 0;
